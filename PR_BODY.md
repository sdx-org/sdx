**Summary**
Replaces ad-hoc `print()` with `logging` + `RichHandler` in `research/backend/cli.py`, exposes a safe preview `--dry-run` in `scripts/gen_models/gen_sqla.py`, and adds a brief GSOC onboarding note to `DEVELOPMENT.md`.
...
**Why**
- Low-risk, high-ROI changes to improve developer/contributor experience and create a clear GSOC-friendly starter task.
